var COLLECTIONS = require('../constants/collections');

var Collections = function (PostGre) {
    var Collection = PostGre.Collection.extend({});

    this[COLLECTIONS.USERS] = require('./users')(PostGre, Collection);
    this[COLLECTIONS.POSTS] = require('./posts')(PostGre, Collection);
    this[COLLECTIONS.COUNTRIES] = require('./countries')(PostGre, Collection);
    this[COLLECTIONS.COUNTRIES_SEARCH_COUNT] = require('./countrySearchCount')(PostGre, Collection);
    this[COLLECTIONS.VISITED_COUNTRIES] = require('./visitedCountries')(PostGre, Collection);
    this[COLLECTIONS.COUNTRIES] = require('./countries')(PostGre, Collection);
    this[COLLECTIONS.STATIC_INFO] = require('./staticInfo')(PostGre, Collection);
    this[COLLECTIONS.FEEDBACKS] = require('./feedbacks')(PostGre, Collection);
    this[COLLECTIONS.COMPLAINTS] = require('./complaints')(PostGre, Collection);
    this[COLLECTIONS.IMAGES] = require('./images')(PostGre, Collection);
};
module.exports = Collections;